...
function OnPlayerWin(winnerServerIds, wagerReward)
    local partReward = tonumber(math.floor(wagerReward / #winnerServerIds))

    if Config.AllowWager then
        for _, serverId in pairs(winnerServerIds) do
...
TriggerEvent('es:addCommand', 'leavebasketball', function(source, args, char)
    local Source = source
    RemovePlayerFromAnyOngoingGame(Source)
end, {
	help = "Leave current basketball game!"
})
