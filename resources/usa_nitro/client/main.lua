...
local NITRO_REQUEST_TIMEOUT_SECONDS = 5

local lastNitroRequestTime = 0
...
local function IsNitroControlPressed()
  ...
  return IsControlPressed(0, INPUT_SPRINT)
end
...
local function NitroLoop(lastVehicle)
  ...
  if isRunning and isEnabled then
    if isDriving then
      if not isBoosting then
        ...
        if GetGameTimer() > lastNitroRequestTime + (NITRO_REQUEST_TIMEOUT_SECONDS * 1000) then
          local plate = GetVehicleNumberPlateText(vehicle)
          TriggerServerEvent("nitro:getFuel", plate)
          lastNitroRequestTime = GetGameTimer()
        end
      end
    else
      if not isPurging and isFueled then
  ...
  elseif isBoosting or isPurging then
    ...
    TriggerServerEvent("nitro:saveFuel", GetVehicleNumberPlateText(vehicle), GetNitroFuelLevel(vehicle))
  end

  if isBoosting or isPurging then
    if not isFueled then
      SetVehicleNitroBoostEnabled(vehicle, false)
      SetVehicleLightTrailEnabled(vehicle, false)
      SetVehicleNitroPurgeEnabled(vehicle, false)
      TriggerServerEvent('nitro:__sync', false, false, false)
...
RegisterNetEvent("nitro:setFuel")
AddEventHandler("nitro:setFuel", function(fuelLevel)
  if fuelLevel > 0 then
    local vehicle = GetVehiclePedIsIn(PlayerPedId())
    InitNitroFuel(vehicle)
    SetNitroFuelLevel(vehicle, fuelLevel)
  else
    exports.globals:notify("Out of NOS!")
  end
end)