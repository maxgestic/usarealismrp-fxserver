...
local RACE_START_COOLDOWN_SECONDS = 15
...
-- Race start command cooldown helper
local lastRecordedRaceStartTime = 0

-- Main command for races
RegisterNetEvent("StreetRaces:handleCommand")
AddEventHandler("StreetRaces:handleCommand", function(args)
    ...
    elseif args[1] == "record" then
        ...
        exports.globals:notify("Track recording enabled!", "^3INFO: ^0Track recording mode enabled! Double click around your map to create and remove checkpoints for the race, then type /race save [name]. Then do /race clear to stop recording.")
    elseif args[1] == "save" then
        -- Check name was provided and checkpoints are recorded
        table.remove(args, 1)
        local name = table.concat(args, " ")
        if name ~= nil and #recordedCheckpoints > 0 and name:len() > 0 then
            -- Send event to server to save checkpoints
            TriggerServerEvent('StreetRaces:saveRace_sv', name, recordedCheckpoints)
        else
            exports.globals:notify("Invalid race!", "^3INFO: ^0You must do /race record and record a track and enter a valid name!")
        end
    ...
    elseif args[1] == "start" then
        local hasExceededTimeout = GetGameTimer() - lastRecordedRaceStartTime > RACE_START_COOLDOWN_SECONDS * 1000
        if hasExceededTimeout then
            ...
            if amount then
                ...
                if #recordedCheckpoints > 0 then
                    ...
                    lastRecordedRaceStartTime = GetGameTimer()
                elseif IsWaypointActive() then
                    ...
                    exports.globals:notify("You must set a waypoint or load a recorded race!", "^3INFO: ^0You must set a waypoint or load a recorded race!")
                    return
                end
            ...
            local remainingCooldownSeconds = math.ceil(((RACE_START_COOLDOWN_SECONDS * 1000) - (GetGameTimer() - lastRecordedRaceStartTime)) / 1000)
            exports.globals:notify("Please wait " .. remainingCooldownSeconds .." more seconds!")
        end
    elseif args[1] == "cancel" then
        -- Send cancel event to server
        TriggerServerEvent('StreetRaces:cancelRace_sv')
    else
        TriggerEvent("chatMessage", "", {}, "^3INFO: ^0Race command options are: clear, leave, record, start [betAmount] [delay (optional)], save [name], load [name], list, delete [name], cancel")
        return
    end
end)
