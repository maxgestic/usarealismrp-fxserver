...
AddEventHandler("PlasmaKart:SpawnedKartFunction", function(kart,kartPlate)
	...
	local vehicle_key = {
		name = "Key -- " .. GetVehicleNumberPlateText(kart),
		quantity = 1,
		type = "key",
		owner = "Unknown",
		make = "Unknown",
		model = "Unknown",
		plate = GetVehicleNumberPlateText(kart)
	}
	-- give key to owner
	TriggerServerEvent("garage:giveKey", vehicle_key)
	TriggerServerEvent('mdt:addTempVehicle', GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(kart))), 'Unknown', GetVehicleNumberPlateText(kart), true)
