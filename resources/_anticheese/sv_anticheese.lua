...
local violationDetectionEnabled = true

webhook = GetConvar("modder-log-webhook")
...
AddEventHandler('anticheese:ViolationDetected', function(reason, securityToken)
	if not exports['salty_tokenizer']:secureServerEvent(GetCurrentResourceName(), source, securityToken) then
		return false
	end
	if violationDetectionEnabled then
