const _CSS_notchOffset = '17.5%' // If your phone is using a skin with a notch you can specify the top distance it takes here.
...
const _CSS_QS_NAMESPACE = typeof(QS) === 'object' && QS.Phone ? 'QS' : null

if ((!_CSS_QS_NAMESPACE) || typeof(IsAppJobBlocked) !== 'function')
...
const originalSetupApps = window[_CSS_QS_NAMESPACE] && window[_CSS_QS_NAMESPACE].Phone && window[_CSS_QS_NAMESPACE].Phone.Functions && window[_CSS_QS_NAMESPACE].Phone.Functions.SetupApplications ? window[_CSS_QS_NAMESPACE].Phone.Functions.SetupApplications : null
...
window.CS_STORIES.hookInterface = () => {
    const _CSS_container = jQuery(`
        <div style="display: none" class="cs-stories-app">
    ...
    jQuery('.phone-application-container').prepend(_CSS_container)
...
jQuery(document).on('click', '.phone-application', e => {
    if (window.$(e.currentTarget).data('app') === 'cs-stories')
        setTimeout(() => window.CS_STORIES.show(), 0)
})

let homeClicked = false

jQuery(document).on('click', '.phone-home-button', e => {
    if (!homeClicked)
        return

    homeClicked = true

    if (window.CS_STORIES.isVisible())
        window.CS_STORIES.hide()
})


window.CS_STORIES.onShowStories = () => { // What to do when cs-stories is shown.
    if (window[_CSS_QS_NAMESPACE] && window[_CSS_QS_NAMESPACE].Phone && window[_CSS_QS_NAMESPACE].Phone.Data)
        window[_CSS_QS_NAMESPACE].Phone.Data.currentApplication = 'cs-stories'
    
    homeClicked = false

    jQuery('.phone-home-button').fadeIn(100).css({
        'background-color': 'rgba(0, 0, 0, 0.40)'
    })
}

window.CS_STORIES.onHideStories = () => jQuery('.phone-home-button').click() // What to do when cs-stories is hidden.
