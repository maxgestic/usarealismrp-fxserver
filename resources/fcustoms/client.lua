...
function closeCustoms()
    ...
    --]]
    TriggerServerEvent("customs:saveCarData", Vehicledata, GetVehicleNumberPlateText(currentvehicle)) -- save car customization
...
AddEventHandler("customs:applyCustomizations", function(customizationData)
    currentvehicle = GetVehiclePedIsIn(GetPlayerPed(-1), false)
    SetVehicleColours(currentvehicle, tonumber(customizationData["Colours"][1]), tonumber(customizationData["Colours"][2]))
    SetVehicleExtraColours(currentvehicle, tonumber(customizationData["ExtraColours"][1]), tonumber(customizationData["ExtraColours"][2]))
    SetVehicleNumberPlateTextIndex(currentvehicle,tonumber(customizationData["Plate"]))
    SetVehicleNeonLightsColour(currentvehicle,tonumber(customizationData["NeonColours"][1]),tonumber(customizationData["NeonColours"][2]),tonumber(customizationData["NeonColours"][3]))
    SetVehicleTyreSmokeColor(currentvehicle,tonumber(customizationData["SmokeColours"][1]),tonumber(customizationData["SmokeColours"][2]),tonumber(customizationData["SmokeColours"][3]))
    SetVehicleModKit(currentvehicle,0)
    SetVehicleMod(currentvehicle, 0, tonumber(customizationData["Spoiler"]))
    SetVehicleMod(currentvehicle, 1, tonumber(customizationData["Front Bumper"]))
    SetVehicleMod(currentvehicle, 2, tonumber(customizationData["Rear Bumper"]))
    SetVehicleMod(currentvehicle, 3, tonumber(customizationData["Skirts"]))
    SetVehicleMod(currentvehicle, 4, tonumber(customizationData["Exhaust"]))
    SetVehicleMod(currentvehicle, 5, tonumber(customizationData["Roll Cage"]))
    SetVehicleMod(currentvehicle, 6, tonumber(customizationData["Grille"]))
    SetVehicleMod(currentvehicle, 7, tonumber(customizationData["Hood"]))
    SetVehicleMod(currentvehicle, 8, tonumber(customizationData["Left Fender"]))
    SetVehicleMod(currentvehicle, 9, tonumber(customizationData["Right Fender"]))
    SetVehicleMod(currentvehicle, 10, tonumber(customizationData["Roof"]))
    SetVehicleMod(currentvehicle, 11, tonumber(customizationData["Engine"]))
    SetVehicleMod(currentvehicle, 12, tonumber(customizationData["Brakes"]))
    SetVehicleMod(currentvehicle, 13, tonumber(customizationData["Transmission"]))
    SetVehicleMod(currentvehicle, 14, tonumber(customizationData["Horn"]))
    SetVehicleMod(currentvehicle, 15, tonumber(customizationData["Suspension"]))
    SetVehicleMod(currentvehicle, 16, tonumber(customizationData["Armour"]))
    if customizationData["Turbo"] == "true" then
        ToggleVehicleMod(currentvehicle, 18, true)
    else
        ToggleVehicleMod(currentvehicle, 18, false)
    end
    if customizationData["Tyre Smoke"] == "true" then
        ToggleVehicleMod(currentvehicle, 20, true)
    else
        ToggleVehicleMod(currentvehicle, 20, false)
    end
    if customizationData["Xenon"] == "true" then
        ToggleVehicleMod(currentvehicle, 22, true)
    else
        ToggleVehicleMod(currentvehicle, 22, false)
    end
    SetVehicleWheelType(currentvehicle, tonumber(customizationData["Wheel Type"]))
    SetVehicleMod(currentvehicle, 23, tonumber(customizationData["Wheels"]))
    SetVehicleMod(currentvehicle, 24, tonumber(customizationData["Wheels2"]))
    if customizationData["Variation"] == "true" then
        SetVehicleMod(currentvehicle, 23, GetVehicleMod(currentvehicle, 23), true)-- Vehicle Mod 23
        SetVehicleMod(currentvehicle, 24, GetVehicleMod(currentvehicle, 24), true)-- Vehicle Mod 24
    end
    if customizationData.n0 == "true" then
        SetVehicleNeonLightEnabled(currentvehicle,0, true)
    else
        SetVehicleNeonLightEnabled(currentvehicle,0, false)
    end
    if customizationData.n1 == "true" then
        SetVehicleNeonLightEnabled(currentvehicle,1, true)
    else
        SetVehicleNeonLightEnabled(currentvehicle,1, false)
    end
    if customizationData.n2 == "true" then
        SetVehicleNeonLightEnabled(currentvehicle,2, true)
    else
        SetVehicleNeonLightEnabled(currentvehicle,2, false)
    end
    if customizationData.n3 == "true" then
        SetVehicleNeonLightEnabled(currentvehicle,3, true)
    else
        SetVehicleNeonLightEnabled(currentvehicle,3, false)
    end
    if customizationData.bp == "true" then
        SetVehicleTyresCanBurst(currentvehicle, false)
    else
        SetVehicleTyresCanBurst(currentvehicle, true)
    end
    SetVehicleWindowTint(currentvehicle,tonumber(customizationData["Windows"]))
end)
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--Useful functions
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
