...
local policeGarages = {
	{x = 433.1854, y = -1022.557, z = 28.72664, _x = 463.7611, _y = -1019.764, _z = 28.07861, _heading = 89.102}, -- Mission Row
...
function AddCustomization(menu)
	...
	-- liveries
	local liveries = {}
	for i = 1, GetVehicleLiveryCount(GetVehiclePedIsUsing(GetPlayerPed(-1))) do liveries[i] = i print(i) end
	local livindex = GetVehicleLivery(GetVehiclePedIsUsing(GetPlayerPed(-1))) + 1
	-- tint
	local tint = {}
	...
	local setlivery = UIMenuSliderItem.New('Vehicle Livery', liveries, livindex)
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(0)
		for i = 1, #policeGarages do
			DrawText3Ds(policeGarages[i].x,policeGarages[i].y,policeGarages[i].z, 450, 16, '~g~E~s~ garage   |   ~g~U~s~ customization') -- putting this in a loop gave > 0.04 ms in resmon, should be ok tho...
		end
