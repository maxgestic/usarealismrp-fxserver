local businessData = {
	...
	dbname = 'PDMdealership',
	owners = {
		['steam:11000011a437404'] = 'Raymond Crews'
	},
	managers = {
		['steam:110000114a182f0'] = 'Delano Flacko',
	},
	employees = {},
	businessBalance = 0,
	employeePay = 200,
...
TriggerEvent('businesses:resourceDatabaseCheck', businessData.dbname, businessData)

TriggerEvent('businesses:returnBusinessData', businessData.dbname, function(data)
	businessData = data
end)

RegisterServerEvent('carDealership:toggleMenu')
AddEventHandler('carDealership:toggleMenu', function()
	local userSource = source
	local steamIdentifier = GetPlayerIdentifier(userSource)
	local user = exports["essentialmode"]:getPlayerFromId(userSource)
	for k, v in pairs(businessData.employees) do
		if k == steamIdentifier and v == user.getActiveCharacterData('fullName') then
			TriggerClientEvent('carDealership:triggerMenu', userSource, false, businessData, steamIdentifier, vehicleShopItems)
			return
		end
	end
	for k, v in pairs(businessData.managers) do
		if k == steamIdentifier and v == user.getActiveCharacterData('fullName') then
			TriggerClientEvent('carDealership:triggerMenu', userSource, true, businessData, steamIdentifier, vehicleShopItems)
			return
		end
	end
	for k, v in pairs(businessData.owners) do
		if k == steamIdentifier and v == user.getActiveCharacterData('fullName') then
			TriggerClientEvent('carDealership:triggerMenu', userSource, true, businessData, steamIdentifier, vehicleShopItems)
			return
		end
...
RegisterServerEvent('carDealership:fireEmployee')
AddEventHandler('carDealership:fireEmployee', function(steam, name)
	local userSource = source
	local steamIdentifier = GetPlayerIdentifier(userSource)
	local user = exports["essentialmode"]:getPlayerFromId(userSource)
	local user_name = user.getActiveCharacterData('fullName')
	if IsManager(steamIdentifier, user_name) or IsOwner(steamIdentifier, user_name) then
		for k, v in pairs(businessData.employees) do
			if k == steam and v == name then
				businessData.employees[steam] = nil
				TriggerEvent('businesses:updateBusinessData', businessData.dbname, businessData)
				TriggerClientEvent('usa:notify', userSource, 'You have fired ~y~'..name..'.')
				return
			end
		end
		for k, v in pairs(businessData.managers) do
			if k == steam and v == name then
				businessData.managers[steam] = nil
				TriggerEvent('businesses:updateBusinessData', businessData.dbname, businessData)
				TriggerClientEvent('usa:notify', userSource, 'You have fired ~y~'..name..'.')
				return
			end
...
RegisterServerEvent('carDealership:promoteEmployee')
AddEventHandler('carDealership:promoteEmployee', function(steam, name)
	local userSource = source
	local steamIdentifier = GetPlayerIdentifier(userSource)
	local user = exports["essentialmode"]:getPlayerFromId(userSource)
	local user_name = user.getActiveCharacterData('fullName')
	if IsManager(steamIdentifier, user_name) or IsOwner(steamIdentifier, user_name) then
		for k, v in pairs(businessData.employees) do
			if k == steam and v == name then
				businessData.employees[steam] = nil
				businessData.managers[steam] = name
				TriggerEvent('businesses:updateBusinessData', businessData.dbname, businessData)
				TriggerClientEvent('usa:notify', userSource, 'You have promoted ~y~'..name..'.')
				return
			end
		end
		return
...
RegisterServerEvent('carDealership:addEmployee')
AddEventHandler('carDealership:addEmployee', function(sourceToAdd)
	local userSource = source
	local steamIdentifier = GetPlayerIdentifier(userSource)
	local user = exports['essentialmode']:getPlayerFromId(userSource)
	local user_name = user.getActiveCharacterData('fullName')
	if IsManager(steamIdentifier, user_name) or IsOwner(steamIdentifier, user_name) then
		if GetPlayerName(sourceToAdd) then
			local employee = exports['essentialmode']:getPlayerFromId(sourceToAdd)
			local employee_name = employee.getActiveCharacterData('fullName')
			local employee_steam = GetPlayerIdentifier(sourceToAdd)
			if not IsManager(employee_steam, employee_name) and not IsEmployee(employee_steam, employee_name) and not IsOwner(employee_steam, employee_name) then
				businessData.employees[employee_steam] = employee_name
				TriggerEvent('businesses:updateBusinessData', businessData.dbname, businessData)
				TriggerClientEvent('usa:notify', userSource, 'You have employed ~y~'..employee_name..'.')
				return
			else
				TriggerClientEvent('usa:notify', userSource, 'This player is already employed!')
			end
		else
			TriggerClientEvent('usa:notify', userSource, 'Player not found!')
		end
	end
end)

RegisterServerEvent('carDealership:setEmployeePayrate')
AddEventHandler('carDealership:setEmployeePayrate', function(newPayrate)
	local newPayrate = round(newPayrate, 0)
	local userSource = source
	local steamIdentifier = GetPlayerIdentifier(userSource)
	local user = exports["essentialmode"]:getPlayerFromId(userSource)
	local user_name = user.getActiveCharacterData('fullName')
	if IsManager(steamIdentifier, user_name) or IsOwner(steamIdentifier, user_name) then
		businessData.employeePay = newPayrate
		TriggerEvent('businesses:updateBusinessData', businessData.dbname, businessData)
		TriggerClientEvent('usa:notify', userSource, 'Payrate updated to: ~y~$'..businessData.employeePay)
		return
	end
end)

RegisterServerEvent('carDealership:demoteManager')
AddEventHandler('carDealership:demoteManager', function(steam, name)
	local userSource = source
	local steamIdentifier = GetPlayerIdentifier(userSource)
	local user = exports["essentialmode"]:getPlayerFromId(userSource)
	local user_name = user.getActiveCharacterData('fullName')
	if IsManager(steamIdentifier, user_name) or IsOwner(steamIdentifier, user_name) then
		for k, v in pairs(businessData.managers) do
			if k == steam and v == name then
				businessData.managers[steam] = nil
				businessData.employees[steam] = name
				TriggerEvent('businesses:updateBusinessData', businessData.dbname, businessData)
				TriggerClientEvent('usa:notify', userSource, 'You have demoted ~y~'..name..'.')
			end
		end
	end
end)

RegisterServerEvent('carDealership:withdrawMoney')
AddEventHandler('carDealership:withdrawMoney', function(amount)
	local amount = round(amount, 0)
	local userSource = source
	local steamIdentifier = GetPlayerIdentifier(userSource)
	local user = exports["essentialmode"]:getPlayerFromId(userSource)
	local user_name = user.getActiveCharacterData('fullName')
	if IsManager(steamIdentifier, user_name) or IsOwner(steamIdentifier, user_name) then
		if businessData.businessBalance - amount >= 0 then
			businessData.businessBalance = businessData.businessBalance - amount
			local balance = user.getActiveCharacterData('money')
			user.setActiveCharacterData('money', balance + amount)
			TriggerEvent('businesses:updateBusinessData', businessData.dbname, businessData)
			TriggerClientEvent('usa:notify', userSource, 'You have withdrew: ~y~$'..amount)
			return
		else
			TriggerClientEvent('usa:notify', userSource, '~y~Not enough money in business!')
		end
	end
end)

RegisterServerEvent('carDealership:depositMoney')
AddEventHandler('carDealership:depositMoney', function(amount)
	local amount = round(amount, 0)
	local userSource = source
	local steamIdentifier = GetPlayerIdentifier(userSource)
	local user = exports["essentialmode"]:getPlayerFromId(userSource)
	...
	if IsManager(steamIdentifier, user_name) or IsOwner(steamIdentifier, user_name) then
		if balance - amount >= 0 then
			...
			user.setActiveCharacterData('money', balance - amount)
			TriggerEvent('businesses:updateBusinessData', businessData.dbname, businessData)
			TriggerClientEvent('usa:notify', userSource, 'You have deposited: ~y~$'..amount)
			return
		else
			TriggerClientEvent('usa:notify', userSource, '~y~Not enough money!')
		end
	end
end)

RegisterServerEvent('carDealership:updateShowroomSlot')
AddEventHandler('carDealership:updateShowroomSlot', function(slot, category, index)
	print(category)
	print(index)
	local storage_capacity = vehicleShopItems[category][index].storage_capacity
	local price = vehicleShopItems[category][index].price
	local hash = vehicleShopItems[category][index].hash
	print(storage_capacity, price, hash)
	TriggerClientEvent('carDealership:updateVehicleInShowroom', -1, slot, hash, price, storage_capacity)
	businessData.showroomData[slot].vehicleName = hash
	businessData.showroomData[slot].vehiclePrice = price
	businessData.showroomData[slot].storageCapacity = storage_capacity
	TriggerEvent('businesses:updateBusinessData', businessData.dbname, businessData)
end)

RegisterServerEvent('carDealership:returnShowroomData')
AddEventHandler('carDealership:returnShowroomData', function()
	TriggerClientEvent('carDealership:updateShowroomData', source, businessData.showroomData)
end)

function IsManager(steam, name)
	local user = exports["essentialmode"]:getPlayerFromId(userSource)
	for k, v in pairs(businessData.managers) do
		if k == steam and v == name then
			return true
...
function IsEmployee(steam, name)
	local user = exports["essentialmode"]:getPlayerFromId(userSource)
	for k, v in pairs(businessData.employees) do
		if k == steam and v == name then
			return true
...
vehicleShopItems = {
	["Suvs"] = {
		...
		{make = "Emperor", model = "Habenaro", price = 35700, hash = "habanero", storage_capacity = 200.0},
		{make = "Karin", model = "BeeJay XL", price = 55700, hash = "bjxl", storage_capacity = 200.0},
		{make = "Benefactor", model = "XLS", price = 55700, hash = "xls", storage_capacity = 200.0}
	},
	["Coupes"] = {
		...
		{make = "Ubermacht", model = "Sentinel Classic", price = 65000, hash = "sentinel3", storage_capacity = 135.0},
		...
		{make = "Hijak", model = "Ruston", price = 210000, hash = "ruston", storage_capacity = 90.0},
		{make = "Lampadati", model = "Michelli GT", price = 125000, hash = "michelli", storage_capacity = 110.0},
		{make = "Ubermacht", model = "Zion", price = 40000, hash = "zion", storage_capacity = 145.0},
		{make = "Ocelot", model = "F620", price = 40000, hash = "f620", storage_capacity = 145.0}
	...
	["Trucks"] = {
		{make = "Karin", model = "Rebel", price = 19500, hash = "rebel", storage_capacity = 230.0},
	...
	["Offroads"] = {
		...
		{make = "Nagasaki", model = "Street Blazer", price = 90000, hash = "blazer4", storage_capacity = 70.0}
	},
	["Motorcycles"] = {
	...
	["Vans"] = {
		...
		{make = "Zirconium", model = "Journey", price = 55000, hash = "journey", storage_capacity = 350.0}
	},
	["Sports"] = {
		...
		{make = "Lampadati", model = "Tropos Rallye", price = 190000, hash = "tropos", storage_capacity = 100.0},
		{make = "Benefactor", model = "Feltzer", price = 215000, hash = "feltzer2", storage_capacity = 100.0}
	},
	["Supers"] = {
		...
		{make = "Progen", model = "Itali GTB", price = 1250000, hash = -2048333973, storage_capacity = 100.0},
		{make = "Progen", model = "Itali GTB Custom", price = 1650000, hash = "italigtb2", storage_capacity = 100.0},
		...
		{make = "Pegassi", model = "Tezeract", price = 3309000, hash = "tezeract", storage_capacity = 110.0},
		{make = "Grotti", model = "Visione", price = 3409000, hash = "visione", storage_capacity = 110.0},
		{make = "Overflod", model = "Entity XXR", price = 3305000, hash = "entity2", storage_capacity = 110.0},
		{make = "Overflod", model = "Entity XF", price = 3000000, hash = "entityxf", storage_capacity = 110.0},
		{make = "Grotti", model = "X80 Proto", price = 3500000, hash = "prototipo", storage_capacity = 110.0},
		{make = "Dewbauchee", model = "Vagner", price = 3900000, hash = "vagner", storage_capacity = 110.0}
	},
	["Classic"] = {
	...
	["Specials"] = {
		...
		{make = "MTL", model = "Pounder", price = 250000, hash = 2112052861, storage_capacity = 900.0},
		...
		{make = "Vapid", model = "Benson", price = 150000, hash = "benson", storage_capacity = 500.0},
	...
	["Electric"] = {
		...
		{make = "Coil", model = "Voltic", price = 95550, hash = "voltic", storage_capacity = 135.0},
		{make = "Pfister", model = "Neon", price = 105550, hash = "neon", storage_capacity = 135.0},
		{make = "Cheval", model = "Surge", price = 65550, hash = "surge", storage_capacity = 135.0},
		{make = "Coil", model = "Cyclone", price = 250000, hash = "cyclone", storage_capacity = 135.0}
	}
}