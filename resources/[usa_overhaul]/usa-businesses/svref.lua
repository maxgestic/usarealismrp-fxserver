...
AddEventHandler("properties:checkSpawnPoint", function(character)
	local spawn = character.get("spawn")
	if type(spawn) ~= "nil" then
		print("spawn existed! checking if spawn is still valid after evictions!")
    -- faster look up --
    if spawn.name then
      if PROPERTIES[spawn.name] then
        if PROPERTIES[spawn.name].owner.identifier == GetPlayerIdentifiers(source)[1] then
          print("Still owns property, leaving spawn! 1")
        else
          if PROPERTIES[spawn.name].coowners then
            for i = 1, #PROPERTIES[spawn.name].coowners do
              if PROPERTIES[spawn.name].coowners[i].identifier == GetPlayerIdentifiers(source)[1] then
                print("Still owns property, leaving spawn! 1")
                return
              end
            end
      ...
      -- for backwards compatability (with spawns that have no .name property set yet) --
  		for name, info in pairs(PROPERTIES) do
            ...
            spawn.name = name
            character.set("spawn", spawn)
  				else
            if info.coowners then
              for i = 1, #info.coowners do
                if info.coowners[i].identifier == GetPlayerIdentifiers(usource)[1] then
                  print("Still owns property, leaving spawn! 2")
                  return
                end
              end
...
AddEventHandler("properties:getUserItemsToStore", function(user_source)
	...
	local inventory = exports["usa-characters"]:GetCharacterField(userSource, "inventory")
	TriggerClientEvent("properties:setItemsToStore", userSource, inventory)
end)
...
AddEventHandler("properties:storeMoney", function(name, amount)
	...
	if user_money - amount >= 0 then
		-- remove from player --
		player.set("money", user_money - amount)
		-- add to property --
...
AddEventHandler("properties:store", function(name, item, quantity)
  ...
  if not had_already then
    table.insert(PROPERTIES[name].storage.items, item)
  end
  -- save property --
...
AddEventHandler("properties:storeVehicle", function(property_name, plate)
  ...
  local owns = false
  local uvehicles = exports["usa-characters"]:GetCharacterField(usource, "vehicles")
  ...
  if owns then
    ...
    TriggerEvent('es:exposeDBFunctions', function(db)
      db.updateDocument("vehicles", plate, { stored_location = property_name }, function()
...
AddEventHandler("properties:retrieveVehicle", function(property_name, vehicle) -- IMPLEMENT
  local usource = source
  TriggerEvent('es:exposeDBFunctions', function(db)
    db.updateDocument("vehicles", vehicle.plate, { stored_location = "" }, function(err)
...
AddEventHandler("properties:retrieve", function(name, item, quantity)
	...
	if player then
		item.quantity = quantity
		if player.canHoldItem(item) then
...
AddEventHandler("properties:purchaseProperty", function(property)
	...
	if GetNumberOfOwnedProperties(ident) < MAX_NUM_OF_PROPERTIES_SINGLE_PERSON then
      ...
      print("set property type to business")
    end
    ...
    if user_money >= PROPERTIES[property.name].fee.price then
			...
			-- subtract money --
			player.set("money", user_money - PROPERTIES[property.name].fee.price)
      -- save property --
...
TriggerEvent('es:addGroupCommand', 'addproperty', 'admin', function(source, args, char)
	local usource = source
