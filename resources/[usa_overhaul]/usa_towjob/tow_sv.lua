	...
	if char.get("job") == "tow" then
		...
		if drivers_license then
			...
			if drivers_license.status == "valid" then
				print("TOW: " .. GetPlayerName(source) .. "["..GetPlayerIdentifier(source).."] is now ON-DUTY for TOW")
				Citizen.CreateThread(function()
					TriggerClientEvent("towJob:showHelpText", source, "Use ~y~/dispatch [id] [msg]~s~ to respond to a tow request!")
					Citizen.Wait(6000)
					TriggerClientEvent("towJob:showHelpText", source, "Use ~y~/tow~s~ when facing a vehicle to load/unload it from the flatbed.")
					Citizen.Wait(6000)
					TriggerClientEvent("towJob:showHelpText", source, "Use ~y~/ping [id]~s~ to request a person\'s location.")
					Citizen.Wait(6000)
					TriggerClientEvent("towJob:showHelpText", source, "A tow truck has been equipped for you just there, use this to tow vehicles.")
				end)
				char.set("job", "tow")
				TriggerClientEvent("towJob:onDuty", source, truckSpawnCoords)
				return
			else
