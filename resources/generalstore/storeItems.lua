storeItems = {
    ["Food"] = {
      {name = "Tuna Sandwhich", price = 50, type = "food", substance = 25.0, quantity = 1, legality = "legal", weight = 8}
    },
    ["Drinks"] = {
      {name = "Water", price = 30, type = "drink", substance = 25.0, quantity = 1, legality = "legal", weight = 8}
    },
    ["Vehicle"] = {
        { name = "Repair Kit", price = 1500, type = "vehicle", quantity = 1, legality = "legal", weight = 20}
    },
    ["Misc"] = {
        { name = "Cell Phone", price = 750, type = "misc", quantity = 1, legality = "legal", contacts = {}, conversations = {}, weight = 10}
    }
}
