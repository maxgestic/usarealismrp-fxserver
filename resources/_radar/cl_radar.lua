    ...
    if ( IsPedSittingInAnyVehicle( ped ) ) then 
        ...
        if ( GetVehicleClass( vehicle ) == 18 ) then
            ...
            if ( radarEnabled ) then 
                Notify( "Radar has been ~g~enabled~s~." )
            else 
                Notify( "Radar has been ~r~disabled~s~." )
            end 
        ...
        while true do 
            if ( UpdateOnscreenKeyboard() == 1 ) then 
                ...
                if ( string.len( speedStr ) > 0 ) then 
                    ...
                    if ( speed < 150 and speed > 20 ) then 
                        TriggerEvent( 'wk:changeRadarLimit', speed )
                        Notify('Speed lock set to ~r~'..speed..' '..radarInfo.speedType..'~s~.')
                    else
                        Notify('Speed lock value error!')
                    end 
...
function ToggleSpeedType()
    if ( radarInfo.speedType == "mph" ) then 
        ...
        Notify( "Radar speed type set to km/h." )
    else 
        radarInfo.speedType = "mph"
        Notify( "Radar speed type set to mi/h." )
    end
end 
...
function ToggleLockBeep()
    if ( radarInfo.lockBeep ) then 
        ...
        Notify( "Radar speed lock sound ~r~disabled~s~." )
    else 
        radarInfo.lockBeep = true
        Notify( "Radar speed lock sound ~g~enabled~s~." )
    end    
end 
                    ...
                    if ( DoesEntityExist( fwdVeh ) and IsEntityAVehicle( fwdVeh ) ) then 
                        ...
                        if ( fwdVehSpeed > radarInfo.fastLimit and not radarInfo.fwdFastLocked ) then 
                            if ( radarInfo.lockBeep ) then 
                                ...
                                lastStreetHASH = GetStreetNameAtCoord(x, y, z)
                                lastStreetNAME = GetStreetNameFromHashKey(lastStreetHASH)
                                PlaySoundFrontend( -1, "Beep_Red", "DLC_HEIST_HACKING_SNAKE_SOUNDS", 1 )
                                TriggerEvent("chatMessage", "", { 0, 0, 0 }, "^1^*[FRONT RADAR] ^r^7"..GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(fwdVeh))).. "^r with plate ".. GetVehicleNumberPlateText(fwdVeh).. "^r clocked at "..fwdVehSpeed..' '..radarInfo.speedType.."^r on "..lastStreetNAME.."^r.")
                            end 
                    ...
                    if ( DoesEntityExist( bwdVeh ) and IsEntityAVehicle( bwdVeh ) ) then
                        ...
                        if ( bwdVehSpeed > radarInfo.fastLimit and not radarInfo.bwdFastLocked ) then 
                            if ( radarInfo.lockBeep ) then 
                                ...
                                lastStreetNAME = GetStreetNameFromHashKey(lastStreetHASH)
                                PlaySoundFrontend( -1, "Beep_Red", "DLC_HEIST_HACKING_SNAKE_SOUNDS", 1 )
                                TriggerEvent("chatMessage", "", { 0, 0, 0 }, "^1^*[REAR RADAR] ^r^7"..GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(bwdVeh))).. "^r with plate ".. GetVehicleNumberPlateText(bwdVeh).. "^r clocked at "..bwdVehSpeed..' '..radarInfo.speedType.."^r on "..lastStreetNAME..'^r.')
                            end 
...
Citizen.CreateThread( function()
    while true do
        ...
        if IsControlPressed( 1, 36 ) and IsControlJustPressed( 1, 311 ) and GetLastInputMethod(0) then
        ...
        if ( IsControlPressed( 1, 19 ) and IsControlJustPressed( 1, 244 ) and GetLastInputMethod(0)) then
