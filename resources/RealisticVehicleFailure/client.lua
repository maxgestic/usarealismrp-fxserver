...
RegisterNetEvent('carDamage:repairKit')
AddEventHandler('carDamage:repairKit', function(target_vehicle)
		if GetVehicleEngineHealth(target_vehicle) < cfg.cascadingFailureThreshold + 5 then
			...
			-- attempt to repair --
			--if GetVehicleOilLevel(target_vehicle) > 0 then
				SetVehicleUndriveable(target_vehicle,false)
				SetVehicleEngineHealth(target_vehicle, cfg.cascadingFailureThreshold + 5)
...
Citizen.CreateThread(function()
	while true do
		...
		if isPedDrivingAVehicle() then
			...
			lastVehicle=vehicle
			if cfg.randomTireBurstInterval ~= 0 and GetEntitySpeed(vehicle) > 70 then tireBurstLottery() end
		else
			if pedInSameVehicleLast == true then
