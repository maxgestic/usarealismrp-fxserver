...
RegisterNetEvent("bcso:setciv")
AddEventHandler("bcso:setciv", function(character, playerWeapons)
...
-- Set up main menu --
---------------------------
local _bcsoMenuPool = NativeUI.CreatePool()
local bcsoMainMenu = NativeUI.CreateMenu("BCSO", "~b~Blaine County Sheriff's Office", 0 --[[X COORD]], 320 --[[Y COORD]])
_bcsoMenuPool:Add(bcsoMainMenu)

TriggerServerEvent("bcso:getWeapons")

RegisterNetEvent("bcso:getWeapons")
AddEventHandler("bcso:getWeapons", function(weps)
    weapons = weps
    CreateWeaponsMenu(bcsoMainMenu)
end)

function CreateUniformMenu(menu)
    local ped = GetPlayerPed(-1)

    local submenu2 = _bcsoMenuPool:AddSubMenu(menu, "Outfits", "Save and load outfits", true)
    ...
    submenu2.SubMenu.OnItemSelect = function(sender, item, index)
      ...
      TriggerServerEvent("bcso:saveOutfit", character, selectedSaveSlot)
        elseif item == loadconfirm then
            DoScreenFadeOut(500)
            Citizen.Wait(500)
            TriggerServerEvent('bcso:loadOutfit', selectedLoadSlot)
  ...
  -- Components --
  local submenu = _bcsoMenuPool:AddSubMenu(menu, "Components", "Modify components", true --[[KEEP POSITION]])
  ...
  -- Props --
  local submenu = _bcsoMenuPool:AddSubMenu(menu, "Props", "Modify props", true --[[KEEP POSITION]])
  for i = 1, 3 do
  ...
  item.Activated = function(parentmenu, selected)
    TriggerServerEvent("bcso:offduty")
...
function CreateWeaponsMenu(menu)
    local submenu = _bcsoMenuPool:AddSubMenu(menu, "Weapons", "Select speciality weapons", true --[[KEEP POSITION]])
     for i = 1, #weapons do
         local item = NativeUI.CreateItem(weapons[i].name, "")
         item.Activated = function(parentmenu, selected)
            TriggerServerEvent("bcso:checkRankForWeapon", weapons[i])
         end
         submenu.SubMenu:AddItem(item)
     end
end
...
function CreateVehiclesMenu(menu)
    local submenu = _bcsoMenuPool:AddSubMenu(menu, "Vehicles", "Deploy a vehicle", true --[[KEEP POSITION]])
    for i = 1, #vehicles do
        local item = NativeUI.CreateItem(vehicles[i].name, "")
        item.Activated = function(parentmenu, selected)
          TriggerServerEvent("bcso:spawnVehicle", vehicles[i])
        end
        submenu.SubMenu:AddItem(item)
    end
end
...
Citizen.CreateThread(function()
    ...
    while true do
        Citizen.Wait(0)
        _bcsoMenuPool:MouseControlsEnabled(false)
        _bcsoMenuPool:ControlDisablingEnabled(false)
        _bcsoMenuPool:ProcessMenus()
        local mycoords = GetEntityCoords(GetPlayerPed(-1))
        -- see if close to any stores --
        if IsControlJustPressed(1, MENU_KEY) and not _bcsoMenuPool:IsAnyMenuOpen() then
            for i = 1, #PRISON_GUARD_SIGN_IN_LOCATIONS do
                if Vdist(mycoords, PRISON_GUARD_SIGN_IN_LOCATIONS[i].x, PRISON_GUARD_SIGN_IN_LOCATIONS[i].y, PRISON_GUARD_SIGN_IN_LOCATIONS[i].z) < 2.0 then
                    TriggerServerEvent("bcso:checkWhitelist", PRISON_GUARD_SIGN_IN_LOCATIONS[i])
                end
            end
        end
        ...
        if closest_location then
            if Vdist(mycoords.x, mycoords.y, mycoords.z, closest_location.x, closest_location.y, closest_location.z) > 1.3 then
                if _bcsoMenuPool:IsAnyMenuOpen() then
                    closest_location = nil
                    _bcsoMenuPool:CloseAllMenus()
                end
            end
        end
    end
...
RegisterNetEvent("bcso:spawnVehicle")
AddEventHandler("bcso:spawnVehicle", function(veh)
  SpawnVehicle(veh)
end)

RegisterNetEvent("bcso:open")
AddEventHandler("bcso:open", function(loc)
    bcsoMainMenu:Clear()
    CreateUniformMenu(bcsoMainMenu)
    if loc.vehSpawn then
        CreateVehiclesMenu(bcsoMainMenu)
    end
    CreateWeaponsMenu(bcsoMainMenu)
    bcsoMainMenu:Visible(not bcsoMainMenu:Visible())
    closest_location = loc
    _bcsoMenuPool:RefreshIndex()
end)

RegisterNetEvent("bcso:uniformLoaded")
AddEventHandler("bcso:uniformLoaded", function(uniform)
...
RegisterNetEvent("bcso:close")
AddEventHandler("bcso:close", function()
    _bcsoMenuPool:CloseAllMenus()
end)
...
-- WEAPON SPAWNING --
---------------------
RegisterNetEvent("bcso:equipWeapon")
AddEventHandler("bcso:equipWeapon", function(weapon)
