...
RegisterServerEvent("bcso:getWeapons")
AddEventHandler("bcso:getWeapons", function()
	TriggerClientEvent("bcso:getWeapons", source, WEAPONS)
end)

RegisterServerEvent("bcso:checkWhitelist")
AddEventHandler("bcso:checkWhitelist", function(loc)
	local char = exports["usa-characters"]:GetCharacter(source)
	if char.get("bcsoRank") > 0 then
		TriggerClientEvent("bcso:open", source, loc)
	else
		TriggerClientEvent("usa:notify", source, "~y~You are not whitelisted for BCSO. Apply at https://www.usarrp.gg.")
	end
end)

RegisterServerEvent("bcso:offduty")
AddEventHandler("bcso:offduty", function()
	print("offduty")
	local char = exports["usa-characters"]:GetCharacter(source)
	...
	TriggerClientEvent("bcso:setciv", source, appearance, playerWeapons)
	-----------------------------------
	-- change back into civ clothing --
	-----------------------------------
	--print("closing DOC menu!")
	TriggerClientEvent("bcso:close", source)
	TriggerClientEvent("ptt:isEmergency", source, false)
	TriggerClientEvent("radio:unsubscribe", source)
end)

RegisterServerEvent("bcso:forceDuty")
AddEventHandler("bcso:forceDuty", function()
	local char = exports["usa-characters"]:GetCharacter(source)
	local job = char.get("job")
	if job ~= "bcso" then
		----------------------------
		...
		----------------------------
		char.set("job", "bcso")
		TriggerClientEvent("thirdEye:updateActionsForNewJob", source, "bcso")
		TriggerEvent("bcso:loadUniform", 1, source)
...
AddEventHandler("bcso:saveOutfit", function(uniform, slot)
	local usource = source
	local player_identifer = GetPlayerIdentifiers(usource)[1]
	print(player_identifer)
		...
		db.getDocumentByRow(DB_NAME, "identifier" , player_identifer, function(result)
			if type(result) ~= "boolean" then
				...
				db.createDocument(DB_NAME, employee, function()
					print("new co created in bcso db")
				end)
				TriggerClientEvent("usa:notify", usource, "Uniform saved1!")
			end
		end)	
	end)
end)
...
AddEventHandler("bcso:loadOutfit", function(slot, id)
	...
	TriggerEvent('es:exposeDBFunctions', function(usersTable)
		usersTable.getDocumentByRow(DB_NAME, "identifier" , player_identifer, function(result)
			if result and result.uniform then
				TriggerClientEvent("bcso:uniformLoaded", usource, result.uniform[tostring(slot)] or nil)
			end
		end)
	end)
end)

RegisterServerEvent("bcso:spawnVehicle")
AddEventHandler("bcso:spawnVehicle", function(veh)
	local char = exports["usa-characters"]:GetCharacter(source)
	local job = char.get("job")
	if job == "bcso" then
		TriggerClientEvent("bcso:spawnVehicle", source, veh)
	else
...
-- weapon rank check --
RegisterServerEvent("bcso:checkRankForWeapon")
AddEventHandler("bcso:checkRankForWeapon", function(weapon)
	local usource = source
	local char = exports["usa-characters"]:GetCharacter(usource)
	local job = char.get("job")
	if job == "bcso" then
		...
		if char.get("bcsoRank") >= weapon.rank then
			if weapon.name == "Spike Strips" then
				...
				if char.canHoldItem(weapon) then
					...
					TriggerClientEvent("bcso:equipWeapon", usource, weapon)
