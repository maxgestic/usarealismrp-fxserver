...
Citizen.CreateThread(function()
	while true do
        Citizen.Wait(3)
        local coords = GetEntityCoords(PlayerPedId())
		local dist = GetDistanceBetweenCoords(coords, _wheelPos.x, _wheelPos.y, _wheelPos.z, true)
        if dist < 5.0 and not _isRolling then
			globals:DrawText3D(_wheelPos.x, _wheelPos.y, _wheelPos.z, "[E] - Spin Wheel!")
